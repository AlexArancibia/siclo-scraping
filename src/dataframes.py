...
def create_excel_in_memory(df_disciplines, df_schedules, df_prices):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        autofit_excel(writer, ["Disciplinas", "Horarios", "Precios"])
...
if __name__ == "__main__":
    load_dotenv("../.env")
    data = {
        "disciplinas": [{"nombre": "yoga", "descripcion": "hacemos yoga"}],
        ...
        "precios": [{"sede": "Todas", "descripcion_plan": "plan 1", "valor": 1000, "moneda": "PEN", "recurrencia": "anual"}]
    }
    df_disciplines, df_schedules, df_prices = init_dataframes()
    df_disciplines, df_schedules, df_prices = append_scraped_data(df_disciplines, df_schedules, df_prices, "gym1", data)
    df_disciplines, df_schedules, df_prices = append_scraped_data(df_disciplines, df_schedules, df_prices, "gym2", data)
    excel_bytes = create_excel_in_memory(df_disciplines, df_schedules, df_prices)
    exported_file = export_and_upload(df_disciplines, df_schedules, df_prices, "1M10yylyExJjh8hbtJt7iKGxVo1CkdB2H")
    print(exported_file)
